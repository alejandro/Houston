#!/usr/bin/env node

/*
 * server
 * @date:Sun Mar 25 2012 16:06:16 GMT-0600 (CST)
 * @name: server.js
 * @licence: MIT
*/


var http = require('http');
var fs   = require('fs');
var path = require('path');

var params ={}

for (var key in process.argv){
  var attr =process.argv[key];
  if (/--/gi.test(attr) || /-/gi.test(attr)){
    var attri = attr.replace('--','').replace('-','').replace(/=.*/gi,'')
    if (/=.*/gi.test(attr)){
      var path  = attr.split('=')[1];
      params[attri] = path;
    } else {
      params[attri] = process.argv[++key];
    }
  }
}

var cfg ={
  port :8000,
  path : process.env.PWD,
  browser:true
}
function logHelp(){
  console.log(' ')
  console.log(' Houston :: A cool static files server')
  console.log(' ')
  console.log(' Available options:')
  console.log('\t --port, -p \t Listening port to Houston default to 8000 ')
  console.log('\t --path, -d \t Dir of starting point to houston, default to actual dir')
  console.log('\t --browser, -b \t open browser window, (true,false) default to true')
  console.log('\t --help, -h \t show this info')
  console.log(' ')
  console.log(' :: end of help ::')
}
Object.keys(params).map(function(key){
  switch (key){
    case 'port':
    case 'p':
      cfg['port'] = params[key];
      break;
    case 'path':
    case 'd':
      if (params[key]=='.'|| params[key]=='')
        cfg['path'] = process.env.PWD;
      else 
        cfg['path']=params[key];
      break;
    case 'browser':
    case 'b':
      cfg['broser'] =params[key];
    case 'v':
    case 'version':
      console.log('Houston '+ require(__dirname+'/../package.json').version)
      process.kill(0)
    case 'help':
    case 'h':
      logHelp();
      process.kill(0);
    default:
     break;
  }
})
var colors ={
  'bold'      : ['\033[1m',  '\033[22m'],
  'cyan'      : ['\033[36m', '\033[39m'],
  'green'     : ['\033[32m', '\033[39m'],
  'magenta'   : ['\033[35m', '\033[39m'],
  'red'       : ['\033[31m', '\033[39m'],
  'yellow'    : ['\033[33m', '\033[39m']
}
  

var csl = function(){
  var s = colors.bold[0]+colors.green[0]+'⢷⡾ server ➔'+colors.green[1]+colors.bold[1]
  var s2 = colors.bold[0]+colors.red[0]+'⢷⡾ server ➔'+colors.red[1]+colors.bold[1]
  this.log = function(msg){
    console.log(s,msg)
  }
  this.warn = function(msg){
    console.warn(s2, msg)
  }
}
csl = new csl

var readDir = function(location){
  var template = fs.readFileSync(path.join(__dirname,'..','public','index.html'),'utf8');
  var s = fs.readdirSync(location);
  var listOfFiles ='<ul class="list"><li><h2> Empty Dir</h2></li></ul>'
  if (s.length >=1){
    listOfFiles = '<ul class="list"><li class="return"><a href="#" id="return">return...</a></li>'
    listOfFiles += s.sort().map(function(file){
      if (file[0]!=='.'){
        var relative = path.join(location,file).replace(cfg.path,'');
        var absolute = path.join(cfg.path,relative);
        
        if (path.existsSync(absolute)){
          if (fs.statSync(absolute).isDirectory()){
            relative[0]=='/' ? relative = relative.substr(1) : relative;
            return '<li class="dir"><a href="/'+relative+'">'+file+'</a></li>';
          } else {
            var ext = path.extname(absolute).substr(1);
            relative[0]=='/'?relative = relative.substr(1):relative;
            return '<li class="'+ ext+'"><a href="/'+relative+'">'+file+'</a></li>';
          } 
        } else {
            return '';
        }
      }else{ return ''; }
    }).join('');
    listOfFiles += '</ul>';
    return template.replace('{list}',listOfFiles).replace('{title}',location).replace('{title}',location);
  } else {
    return template.replace('{list}',listOfFiles).replace('{title}',location).replace('{title}',location);
  }
  
}
var app  = http.createServer(function(req,res){
  req.url = unescape(req.url);
  var absolute =path.join(cfg.path,req.url);
  var isPublic = req.url.substr(0,8)=='/public/';
  if (req.url=='/favicon.ico') {
    res.end(fs.readFileSync(__dirname+'/../public/img.png'));
  }
  if (isPublic) var rabpub = path.join(__dirname,'..',req.url);
  if (req.url === '/'){
    var location = path.join(cfg.path)
    res.writeHeader(200,{'Content-type':'text/html'})
    res.end( readDir(location));
  } else if (req.url !=='/favicon.icon' && !isPublic){
    if (path.existsSync(absolute)){
      if (fs.statSync(absolute).isDirectory()){
        res.writeHeader(200,{'Content-type':'text/html'})
        res.end(readDir(absolute));
      } else {
       if (path.extname(absolute) =='.html'){
         res.writeHeader(200,{'Content-type':'text/html'});
        }
        res.end(fs.readFileSync(absolute, 'utf8'));
     }
    } else {
      res.writeHeader(404)
      res.end('<not found>');
    }
  } else if (isPublic && path.existsSync(rabpub) && !fs.statSync(rabpub).isDirectory()) {
    res.end(fs.readFileSync(rabpub));
  }else if(isPublic) {
    res.end(fs.readFileSync(__dirname+'/../public/default.png'));
  } else {
    res.writeHeader(404)
    res.end('<not found>');
  }
});

app.listen(cfg.port, function() {
  csl.log('Server running on port => '+cfg.port);
});
for (var key in cfg){
  csl.log(key +' : ' + cfg[key]);
}

if (cfg.browser){
  var browser;
  switch (process.platform) {
    case "win32": browser = "start"; break;
    case "darwin": browser = "open"; break;
    default: browser = "xdg-open"; break;
  }
}
csl.warn('Opening a new browser window...')
require('child_process').spawn(browser, ['http://localhost:'+cfg.port]);