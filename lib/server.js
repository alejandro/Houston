...
function readContents (location, req, res) {
  var dir = fs.readdirSync(location)
    , showIndex = req.query.list || req.query.l || req.query.index || req.query.i;
...
function server (req, res) {
  ...
  //
  // Emit the method to the parent.
  // I don't know what i'm doing xD
  // So you can do:
  // var myapp = require('nhouston')(options);
  // myapp.on('POST', function(req,res){ /* my codez */});
  //
  if (req.method != 'GET' && this.listeners(req.method).length) {
    return this.emit(req.method, req, res);
  ...
  var absolute = path.join(cfg.path, req.url)
    ...
    , rabpub = ''
    , notFound = '<div style="text-align:center"><h1> 404 - Not found </h1><hr>' +
                 '<code> node-' + process.version + ' running on ' + cfg.port + '</code></div>';

  req.isPublic = isPublic;

  req.once('error', function(){
    res.writeHeader(500, {'Content-type':'text/plain'});
    res.end(notFound.replace('404 - Not found','500 - Internal Server Error'));
  });
...
function houston (options) {
  ...
  if (options.spdy) app = require('spdy').createServer(options.spdy, server);
  else if (options.https) app = require('https').createServer(options.https, server);
  else app = require('http').createServer(server);
